...
{
    public class AutoGetEnergyConsumptionRuntime_V1
    {
        ...
        public AutoGetEnergyConsumptionRuntime_V1(SqlServerDataAdapter.ISqlServerDataFactory mySqlServerDataFactory)
        {
        ...
        public DataTable GetOrganizationContrast(string myLevelCode)
        {
            string m_Sql = @"select 
                                ...
                                (case when B.LevelType = 'ProductionLine' then C.LevelCode else B.LevelCode end) as LevelCode 
                                from analyse_KPI_OrganizationContrast A
								left join system_Organization C on A.ClinkerOrganizationID = C.OrganizationID
            ...
            try
            {
                ...
                string m_Sql = @"select 
                                ...
                                union all
                                    select C.LevelCode as LevelCode,
                                    {3} as VariableId, 
                                    B.CumulantDay as Value
                                    from RealtimeIncrementCumulant B, system_Organization C
                                    where B.UpdateDateTime >= '{0}'
                                    and B.UpdateDateTime <='{1}' + ' 23:59:59'
                                    and B.OrganizationID = C.OrganizationID
                                    and C.LevelCode like '{2}%'
                                    and C.Type = '{4}'
                                ) D
        ...
        private void GetCementPackingDataValue(string myStartTime, string myEndTime, ref DataTable myValueTable, string myFactoryOrganizationId, string myStaticsCycle)
        {
            string m_Sql = @"select 
                                ...
                                from (
                                    ...
                                    and B.VariableId = 'cementPacking_ElectricityQuantity'
                                union all
                                    select B.VariableId as VariableId, 
                                    B.CumulantDay as Value
                                    from RealtimeIncrementCumulant B
                                    where B.UpdateDateTime >= '{0}'
                                    and B.UpdateDateTime <='{1}' + ' 23:59:59'
                                    and B.OrganizationID in ({2})
                                    and B.VariableId = 'cementPacking_ElectricityQuantity'
                             ...
                             select 
                                ...
                                from (
                                    ...
                                    and B.VariableId = 'cement_CementOutput'
                                union all 
                                    select B.VariableId as VariableId, 
                                    B.CumulantDay as Value
                                    from RealtimeIncrementCumulant B,system_Organization A, system_Organization C
                                    where A.OrganizationID in ({2})
									and C.LevelCode like A.LevelCode + '%'
									and A.OrganizationID <> C.OrganizationID
                                    and B.UpdateDateTime >= '{0}'
                                    and B.UpdateDateTime <='{1}' + ' 23:59:59'
